package lotto.common;

/**
 * packageName : lotto.common
 * fileName : Constants
 * author : haedoang
 * date : 2021-11-05
 * description : 공통 상수 정의 클래스
 */
public class Constants {


    public static final String MSG_INPUT_PURCHASE_PRICE = "구입금액을 입력해 주세요.";
    public static final String MSG_OUTPUT_PURCHASE_RESULT_SUFFIX = "개를 구매했습니다.";
    public static final String MSG_INPUT_LAST_WINNING_NUMBERS = "지난 주 당첨 번호를 입력해 주세요.";
    public static final String MSG_INPUT_BONUMS_NUMBER = "보너스 볼을 입력해 주세요.";
    public static final String MSG_OUTPUT_LOTTO_RESULT = "당첨통계";
    public static final String MSG_OUTPUT_LINE_SEPARATOR = "---------";
    public static final String MSG_OUTPUT_PLACE_PREFIX = "개 일치 (";
    public static final String MSG_OUTPUT_PLACE_PREFIX_BONUS = "개 일치, 보너스 볼 일치(";
    public static final String MSG_OUTPUT_PLACE_SUFFIX = "원)- ";

    public static final String MSG_OUTPUT_SUFFIX = "개";
    public static final String MSG_OUTPUT_YIELD_PREFIX = "총 수익률은 ";
    public static final String MSG_OUTPUT_YIELD_SUFFIX = "입니다.";

    public static final String NUMBER_SEPARATOR = ",";

    public static final String CONTAINER_PREFIX = "[";
    public static final String CONTAINER_SUFFIX = "]";

}